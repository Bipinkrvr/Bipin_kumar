...
export function ExperienceSection() {
  // State to handle mobile touch interactions
  const [isActive, setIsActive] = useState(false);

  return (
    // FIX 1 & 4: Removed min-h-screen and adjusted padding to eliminate the top gap and layout jerks
    <section id="experience" className="w-full flex flex-col justify-center relative py-12 sm:py-20 px-4 sm:px-8 bg-transparent">
      ...
      <div className="grid gap-8 w-full max-w-6xl mx-auto">
        {/* FIX 3: Added onTouch events and a data attribute to trigger animations on mobile */}
        <div 
        ...
        >
          <Card className="bg-white border-2 border-zinc-300 group-hover:border-cyan-500/70 group-data-[active=true]:border-cyan-500/70 group-hover:shadow-[0_10px_30px_rgba(34,211,238,0.15)] group-data-[active=true]:shadow-[0_10px_30px_rgba(34,211,238,0.15)] transition-all duration-500 rounded-xl overflow-visible">
            ...
            {/* FIX 2: POWER SOURCE - Perfectly centered on top border using top-0 and -translate-y-1/2 */}
            ...
            {/* FIX 2: THE SWITCH (RELAY) - Perfectly centered on top border */}
            ...
            {/* FIX 2: THE BULB (LOAD) - Perfectly centered on bottom border using bottom-0 and translate-y-1/2 */}
            ...
            {/* FIX 2: GROUND (GND) - Perfectly centered on bottom border */}
            ...
            <CardContent className="space-y-4 pb-10 sm:pb-12 px-5 sm:px-8">
              <ul className="space-y-3 font-mono text-xs sm:text-sm text-zinc-700">
                <li className="flex gap-3">
                  ...
                  <span>Implemented standardized UI designs and front-end frameworks to scale the platform architecture.</span>
                </li>
                <li className="flex gap-3">
                  <Terminal className="w-4 h-4 text-zinc-400 group-hover:text-emerald-500 group-data-[active=true]:text-emerald-500 transition-colors shrink-0 mt-0.5" />
                  <span>Developed highly responsive pages, integrating fluid user animations and core branding elements.</span>
                </li>
                <li className="flex gap-3">
                  <Terminal className="w-4 h-4 text-zinc-400 group-hover:text-emerald-500 group-data-[active=true]:text-emerald-500 transition-colors shrink-0 mt-0.5" />
                  <span>Collaborated directly with cross-functional nodes via Microsoft Teams to ensure seamless deployment protocols.</span>
                </li>
              </ul>
            </CardContent>
          </Card>
